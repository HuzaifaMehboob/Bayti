  ...
  return (
    <div className="w-[90%] xl:w-[1224px] mx-auto mt-24 mb-40">
      ...
      <div className="max-w-fit text-center mx-auto mb-10">
        <h3 className="heading-3">{t("consultantsSection.heading")}</h3>
      ...
      <div className="flex flex-col lg:flex-row  justify-between gap-6">
        {cards?.map(
          (card: { title: string }, index: number) => (
            <div
              key={index}
              className={`w-full lg:max-w-[404px] border border-gray-200 h-[150px] px-4 flex items-center py-6 gap-[28px] bg-white rounded-xl shadow-sm ${i18n.language === "ar" ? "text-right" : "text-left"}`}
            >
